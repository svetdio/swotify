...
def execute_train(csv_url):
    ...
    try:
        with open("models/retrain/manifest.json", "r") as manifest:
            model_manifest = json.load(manifest)
    except:
        model_manifest = {}

    # data['message'] = f"{model_manifest}"
    ...
    for i, k in enumerate(tuned_metrics_list):
        if abs(k['Testing F1 score'] - k['Training F1 score']) <= 0.02:
            bm = (best_model_container, k['Model Used'], f"Tuned{k['Model Used']}_{now}.pkl", k)
            models_to_be_created.append(bm)
    
    for i, k in enumerate(metrics_list):
        if abs(k['Testing F1 score'] - k['Training F1 score']) <= 0.02:
            bm = (model_container, k['Model Used'], f"Initial{k['Model Used']}_{now}.pkl", k)
            models_to_be_created.append(bm)

    data['message'] = f"Determined {len(models_to_be_created)} best fit models... "
    yield f"event: mlstep\ndata: {json.dumps(data)}\n\n"

    data['message'] = f"Pickling and registering the models..."
    yield f"event: mlstep\ndata: {json.dumps(data)}\n\n"
    
    


    for container, key, filename, k in models_to_be_created:
        ...
        with open(model_pkl, 'wb') as file:
            ...
            model_manifest[filename] = {
                "test_f1": k['Testing F1 score'],
                "acc" : k['Testing Accuracy'],
                "model": key
            }
            data['message'] = f'Saved {key} model in {model_pkl}'
            yield f"event: mlstep\ndata: {json.dumps(data)}\n\n"

    model_manifest = json.dumps(model_manifest, indent=4)
    
    # Writing to model_manifest.json
    with open("models/retrain/manifest.json", "w") as manifest:
        manifest.write(model_manifest)

    data['message'] = f"Pickling and registering the models... COMPLETED"
    yield f"event: mlstep\ndata: {json.dumps(data)}\n\n"
